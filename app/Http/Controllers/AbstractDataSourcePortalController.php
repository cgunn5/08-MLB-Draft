...
abstract class AbstractDataSourcePortalController extends Controller
{
    abstract protected function datasetPortal(): string;

    abstract protected function routeGroup(): string;

    abstract protected function indexViewName(): string;

    abstract protected function profileFeedSlotsRequestKey(): string;

    abstract protected function profileFeedSlotsColumn(): string;

    abstract protected function profileFeedAssignmentsResponseKey(): string;

    abstract protected function profileFeedAssignmentsSlotFieldKey(): string;

    /**
     * @param  Collection<int, DataSourceUpload>|null  $allUploads
     * @return list<string>
     */
    abstract protected function resolvedProfileFeedSlotsForUpload(DataSourceUpload $upload, ?Collection $allUploads = null): array;

    private const GROUP_VALUE_EMPTY_SENTINEL = '__EMPTY__';

    /**
     * HS Data: place the Perfect Game yearly source (linked from the career master) immediately after
     * the career tab so it stays visible when many CSVs exist.
     *
     * @param  Collection<int, DataSourceUpload>  $uploads
     * @return Collection<int, DataSourceUpload>
     */
    protected function orderHsUploadsForLibraryTabs(Collection $uploads): Collection
    {
        $careerRows = $uploads->filter(static fn (DataSourceUpload $u): bool => $u->isCareerPgMaster())->values();
        $career = $careerRows->first();
        $yearlyId = $career?->career_pg_source_upload_id;
        $yearlyRows = ($yearlyId !== null)
            ? $uploads->filter(static fn (DataSourceUpload $u): bool => ! $u->isCareerPgMaster() && (int) $u->id === (int) $yearlyId)->values()
            : collect();
        $rest = $uploads->filter(static function (DataSourceUpload $u) use ($yearlyId): bool {
            if ($u->isCareerPgMaster()) {
                return false;
            }
            if ($yearlyId !== null && (int) $u->id === (int) $yearlyId) {
                return false;
            }
        ...
        return $careerRows->concat($yearlyRows)->concat($rest);
    }

    protected function assertUploadAccess(Request $request, DataSourceUpload $dataSourceUpload): void
    {
        abort_unless($dataSourceUpload->user_id === $request->user()->id, 404);
        abort_unless($dataSourceUpload->dataset_portal === $this->datasetPortal(), 404);
    }
    ...
    public function index(): View
    {
        $user = auth()->user();
        if ($user !== null) {
            DataSourcePortalMisplacementFixer::fixMisplacedFileUploadsForUser($user);
        }
        if ($user !== null && $this->datasetPortal() === DataSourceUpload::PORTAL_HS) {
        ...
        if ($this->datasetPortal() === DataSourceUpload::PORTAL_HS) {
            $uploads = $this->orderHsUploadsForLibraryTabs($uploads);
        }
        ...
        return view($this->indexViewName(), [
            'uploads' => $uploads,
            'initialActiveId' => $initialActiveId,
            'uploadSummaries' => $this->buildDataSourceLibraryUploadSummaries($uploads),
            'dataSourceLibraryProfileFeed' => [
                'profileFeedSlotsSummaryField' => $this->profileFeedAssignmentsSlotFieldKey(),
                'profileFeedSlotsPayloadKey' => $this->profileFeedSlotsRequestKey(),
                'profileFeedAssignmentsResponseKey' => $this->profileFeedAssignmentsResponseKey(),
                'profileFeedAssignmentsEachSlotField' => $this->profileFeedAssignmentsSlotFieldKey(),
            ],
        ]);
    }
     ...
     */
    protected function buildDataSourceLibraryUploadSummaries(Collection $uploads): array
    {
        $slotField = $this->profileFeedAssignmentsSlotFieldKey();

        return $uploads->map(function (DataSourceUpload $u) use ($uploads, $slotField): array {
            $browse = $u->dataset_browse_settings;

            return [
                'id' => $u->id,
                'name' => $u->name,
                'upload_kind' => $u->upload_kind ?? DataSourceUpload::UPLOAD_KIND_FILE,
    ...
    public function store(StoreDataSourceUploadRequest $request): RedirectResponse
    {
        ...
        DataSourceUpload::query()->create([
            ...
            'dataset_portal' => $this->datasetPortal(),
    ...
    public function show(Request $request, DataSourceUpload $dataSourceUpload): RedirectResponse
    {
        ...
        return redirect()->route($this->routeGroup().'.index', [
    ...
    public function destroyUpload(Request $request, DataSourceUpload $dataSourceUpload): JsonResponse|RedirectResponse
    {
        ...
        if ($request->wantsJson()) {
            return response()->json([
                'ok' => true,
                'redirect' => route($this->routeGroup().'.index'),
            ]);
        }
    ...
    public function playerNames(Request $request, DataSourceUpload $dataSourceUpload): JsonResponse
    {
        ...
        try {
            ...
            while (($row = fgetcsv($handle)) !== false) {
                if (DataSourceCsvFileStats::isBlankCsvRow($row)) {
                    continue;
                }
                $raw = isset($row[$playerIdx]) ? trim((string) $row[$playerIdx]) : '';
                if ($raw === '') {
    ...
    public function updateSettings(UpdateDataSourceUploadSettingsRequest $request, DataSourceUpload $dataSourceUpload): JsonResponse
    {
        ...
        if ($request->has('name')) {
            /** @var string $name */
            $name = $request->validated('name');
            $dataSourceUpload->name = trim($name);
        }
        ...
        if ($request->has($profileSlotRequestKey) && ! $dataSourceUpload->isCareerPgMaster()) {
            ...
            $rawSlots = $request->input($profileSlotRequestKey, []);
            $slots = [];
        ...
        if ($request->has('name')) {
            $response['name'] = $dataSourceUpload->name;
        }
        ...
        if ($request->has($profileSlotRequestKey)) {
            $slotFieldKey = $this->profileFeedAssignmentsSlotFieldKey();
            $allForSlots = DataSourceUpload::query()
                ->where('user_id', $dataSourceUpload->user_id)
                ->where('dataset_portal', $this->datasetPortal())
                ->orderBy('id')
                ->get(['id', 'hs_profile_feed_slots', 'ncaa_profile_feed_slots', 'upload_kind', 'career_pg_source_upload_id']);

            $response[$this->profileFeedAssignmentsResponseKey()] = $allForSlots
                ->map(function (DataSourceUpload $u) use ($allForSlots, $slotFieldKey): array {
                    return [
                        'id' => (int) $u->id,
                        $slotFieldKey => $this->resolvedProfileFeedSlotsForUpload($u, $allForSlots),
                    ];
                })
                ->values()
                ->all();
